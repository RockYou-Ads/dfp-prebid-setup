...
from tests_integration.helpers.archive_order_by_name import \
    archive_order_by_name
from tests_integration.helpers.get_advertiser_by_name import \
...
from tests_integration.helpers.get_line_items_for_order import \
    get_line_items_for_order
from tests_integration.helpers.get_order_by_name import get_order_by_name
from tests_integration.helpers.get_placement_by_name import \
    get_placement_by_name
...
class NewPrebidPartnerTests(TestCase):
    ...
    def tearDown(self):
        print(
            'Cleaning up: archiving the order and deleting custom targeting key-values.')
    ...
    def test_new_partner(self, mock_input):
        ...
        sorted_line_items = sorted(line_items, key=lambda li: li['costPerUnit'][
            'microAmount'])
        ...
        for index, li in enumerate(sorted_line_items):
            ...
            custom_targ_logic = targ['customTargeting']['children'][0][
                'children']
            ...
            self.assertEqual(custom_targ_hb_bidder['valueIds'],
                             [expected_bidder_val['id']])
